...
async def test_get_sos_status(client: AsyncClient, device_token: str,
    ...
    # Status check with the ORIGINAL device's token
    from app.core.security import create_device_jwt
    original_device_token = create_device_jwt("test-device-id")
    status_headers = {"Authorization": f"Bearer {original_device_token}"}
    
    status_resp = await client.get(f"/api/v1/sos/{report_uuid}/status", headers=status_headers)
