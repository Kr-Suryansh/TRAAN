...
```json
{
  ...
  // severity_hint removed (handled fully by backend/AI now)
...
**Design intent:** tapping the SOS button alone is a complete, valid emergency signal — `is_quick_sos: true`, `medical_snapshot` auto-filled from the profile, everything else null. Anyone with time/composure can add `custom_message` or set `emergency_type` explicitly, but nothing beyond the tap is ever required.
