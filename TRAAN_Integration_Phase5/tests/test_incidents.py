...
async def test_get_incident_detail(client: AsyncClient, admin_token, cleanup_phase4_data):
    ...
    assert isinstance(data["sos_reports"], list)
...
async def test_dispatch_resource(client: AsyncClient, admin_token, cleanup_phase4_data):
    ...
    assert inc_data["incident"]["status"] == "dispatched"
    assert len(inc_data["incident"]["assigned_resources"]) == 1
    assert inc_data["incident"]["assigned_resources"][0] == resource_id
