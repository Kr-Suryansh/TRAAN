...
# Define a Gemini-specific Flags schema without default values.
# The google-genai SDK STRICTLY forbids default values in the response_schema.
...
def generate_incident_summary(sos_reports: List[SOSRequest]) -> Dict[str, Any]:
    ...
    prompt_lines = [
        ...
        "9. Any cause attribution (e.g., 'earthquake', 'flood', 'fire') mentioned ONLY inside custom messages must be described with uncertainty language (e.g., 'reportedly', 'according to reports', 'the cause is not yet confirmed'). Do NOT state it as confirmed fact unless emergency_type also corroborates it.",
        "",
