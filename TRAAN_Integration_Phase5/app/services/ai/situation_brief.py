...
def get_cached_situation_brief() -> SituationBriefCache:
    ...
    # (In D, refresh_situation_brief requires incidents/resources which are queried async.)
    return SituationBriefCache(
        brief_text="Situation brief is currently being generated. Please check back shortly.",
...
def start_periodic_refresh(interval_seconds: int = 10800):
    """
        ...
        def _worker():
            ...
            async def main_loop():
                while not _STOP_BACKGROUND_EVENT.is_set():
                    try:
                        # In D's architecture, we need an async session to fetch data
                        async def fetch_and_refresh():
                            # Create an explicitly isolated engine strictly for this event loop
                            isolated_engine = create_async_engine(settings.DATABASE_URL, poolclass=NullPool, echo=False)
                            IsolatedSessionLocal = async_sessionmaker(isolated_engine, class_=AsyncSession, expire_on_commit=False)
                            
                            try:
                                async with IsolatedSessionLocal() as db:
                                ...
                                for i in db_incidents:
                                    # Dummy location for brief
                                    from app.models.schemas import Incident as PydanticIncident, Location, SeverityEnum, Flags, RecommendedResource
                                ...
                                for r in db_resources:
                                    from app.models.schemas import Resource as PydanticResource, ResourceCategory, ResourceStatus, Location
                                    p_res = PydanticResource(
                                        ...
                                        location=Location(lat=0.0, lng=0.0, district=r.district),
                                        contact=r.contact,
                                        last_updated_at=r.last_updated_at
                                    )
                                    resources.append(p_res)
                                    
                                refresh_situation_brief(incidents=incidents, resources=resources)
                            finally:
                    ...
                    # Wait asynchronously for the configured interval while
                    # still allowing the stop event to terminate the worker.
                    for _ in range(interval_seconds):
                        if _STOP_BACKGROUND_EVENT.is_set():
                            break
