...
from app.models.sos import BatchUpload, BatchResponse, SOSStatusResponse, SOSStatus
...
async def upload_sos_batch(
    ...
    for report in payload.sos_batch:
        ...
        new_report = SOSReport(
            ...
            status=report.status.value if report.status else SOSStatus.uploaded.value,
...
async def get_sos_status(
    ...
    result = await db.execute(
        select(SOSReport).where(SOSReport.uuid == uuid)
    )
    ...
    return SOSStatusResponse(status=report.status)
