...
async def register_device(
    ...
    # Validate: must be a well-formed UUID4 (422 on invalid — FastAPI convention)
    try:
        _uuid.UUID(request.device_id, version=4)
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="device_id must be a valid UUID4 (generated on first app install)",
        )

    # Idempotent upsert — if the device is already registered, issue a fresh JWT.
    # This handles re-registration after a 401 (GatewaySyncWorker P0.4.5 flow)
    # without creating a duplicate row or changing the canonical device_id.
...
async def authority_login(
    ...
    refresh_token = create_authority_refresh_jwt(authority.id, authority.role)
...
async def authority_refresh(request: AuthorityRefreshRequest):
    ...
    if payload.get("type") != "authority_refresh":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    new_access = create_authority_jwt(authority_id, role)
    new_refresh = create_authority_refresh_jwt(authority_id, role)
    return AuthorityRefreshResponse(access_token=new_access, refresh_token=new_refresh)
