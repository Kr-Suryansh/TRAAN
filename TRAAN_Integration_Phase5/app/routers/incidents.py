...
from app.models.schemas import SOSRequest, Location as SOSLocation
from app.services.incident_pipeline import _max_severity
import asyncio

logger = logging.getLogger(__name__)
router = APIRouter()

async def _fetch_sos_reports(uuids, db: AsyncSession):
    if not uuids:
        return []
    ...
    ).where(SOSReport.uuid.in_(uuids))
...
async def _build_incident_response(row, db: AsyncSession) -> IncidentResponse:
    ...
    return IncidentResponse(
        ...
        source_sos_uuids=inc_obj.source_sos_uuids,
...
async def get_incident(
    ...
    inc_resp = await _build_incident_response(row, db)
    sos_reports = await _fetch_sos_reports(row[0].source_sos_uuids, db)
...
async def refresh_summary(
    ...
    sos_reports_data = await _fetch_sos_reports(inc.source_sos_uuids, db)
    
    # Map to Pydantic SOSRequest schema for the AI service
    pydantic_reports = []
    for r in sos_reports_data:
        # Pydantic model for SOSRequest
        req = SOSRequest(
            uuid=str(r["uuid"]),
    ...
    if ai_result.get("ai_success", False):
        inc.ai_summary = ai_result.get("ai_summary")
        inc.flags = ai_result.get("flags", {})
        inc.estimated_people_affected = ai_result.get("estimated_people_affected")
        
        # OPTION A — Max-severity preservation (approved 2026-08-21):
        # The incident's current severity (inc.severity) is D's protected floor.
        # Gemini may escalate it but must NEVER downgrade it.
        gemini_sev = ai_result.get("severity")
        inc.severity = _max_severity(inc.severity, gemini_sev)
