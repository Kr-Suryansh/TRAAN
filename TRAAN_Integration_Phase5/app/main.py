...
from app.services.ai.situation_brief import start_periodic_refresh, stop_periodic_refresh
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            start_periodic_refresh(interval_seconds=settings.SITUATION_BRIEF_REFRESH_INTERVAL)
            logger.info("Situation brief periodic refresh worker started.")
        except Exception as exc:
            logger.warning("Could not start situation brief refresh worker: %s", exc)
    ...
    try:
        ...
        logger.info("Situation brief periodic refresh worker stopped.")
    except Exception:
        pass
...
# ── CORS ──────────────────────────────────────────────────────────────────────
# Permissive CORS for development — allows any origin (Dashboard, Android proxy,
# Swagger UI, etc.) without needing to enumerate them.
# TODO: Before production, replace ["*"] with a specific origin list and set
#       allow_credentials=True (note: allow_credentials=True is incompatible with
#       allow_origins=["*"] in browsers — use explicit origins in prod).
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
