...
class EnumContractTest {
    ...
    @Test
    fun `SeverityHint apiValue matches contract`() {
        assertEquals("critical", SeverityHint.CRITICAL.apiValue)
        assertEquals("high", SeverityHint.HIGH.apiValue)
        assertEquals("medium", SeverityHint.MEDIUM.apiValue)
        assertEquals("low", SeverityHint.LOW.apiValue)
    }
}
